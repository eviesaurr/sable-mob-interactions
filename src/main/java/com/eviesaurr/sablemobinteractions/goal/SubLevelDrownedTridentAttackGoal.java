...
public class SubLevelDrownedTridentAttackGoal extends Goal {

    private static final int RETARGET_INTERVAL_TICKS = 20;
    private static final float TRIDENT_VELOCITY = 1.6f;
    private static final float TRIDENT_INACCURACY = 1.5f;
    ...
    public SubLevelDrownedTridentAttackGoal(PathfinderMob mob, double searchRadius, double attackRange, int fireCooldownTicks) {
        this.mob = mob;
    ...
    public void tick() {
        ...
        fireTridentAt(worldPos);
    }

    private void fireTridentAt(Vec3 targetPos) {
        if (!(this.mob.level() instanceof ServerLevel level)) return;

        this.mob.swing(InteractionHand.MAIN_HAND);

        ThrownTrident trident = new ThrownTrident(level, this.mob, new ItemStack(Items.TRIDENT));
        trident.setPos(this.mob.getX(), this.mob.getEyeY() - 0.1, this.mob.getZ());

        double dx = targetPos.x - trident.getX();
        double dy = targetPos.y - trident.getY();
        double dz = targetPos.z - trident.getZ();

        trident.shoot(dx, dy, dz, TRIDENT_VELOCITY, TRIDENT_INACCURACY);
